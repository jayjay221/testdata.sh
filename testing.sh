...
unPack () {
  ...
  then
    term "2" \
    "Nelze rozbalit \"$ARCHIV\" do \"$TMPDIR/referencni_archiv\""
  fi
}
...
saveRefOut () {
  ...
  # Projdeme kazdy vystupni ref. soubor
  ...
  # Pri rezimu vlastnich dat ulozime jeste vlastni vystupy
  
  if [ "$MODE" = "custom" ] 
  then
    i=0
    # Projdeme vsechny vlastni vystupy
...
saveRefIn () {
  ...
  # Projdeme kazdy referencni vstup
  for soubor in \
  $(ls -v $TMPDIR/referencni_archiv/CZE/*_in.txt 2>/dev/null) 
  do
...
saveOurOut () {
  ...
  do
    ...
    # Posleme do naseho programu aktualni soubor z pole a vysledek 
    # ulozime do docasneho adresare
    TIMING[i]=$(time ($PROGRAM < $soubor \
    > "$TMPDIR/vysledky/${iter}_myout.txt") \
    2>&1 1>/dev/null) 
    
    # Jmeno vytvoreneho souboru si ulozime do pole
  ...
  if [ "$MODE" = "custom" ] # Rezim vlastnich dat
  then
    # Opakuje se predesla cast kodu, pouze iterujeme pres vlastni 
    # vstupy, lisi se format vystupu a 
    # prirazujeme do jineho pole
    ...
    do
      ...
      TIMINGCUST[i]=$(time ($PROGRAM < $soubor \
      > "$TMPDIR/vysledky/${iter}_cust_myout.txt") \
      2>&1 1>/dev/null)
...
testAgRef () {
  ...
  # notace '${#foo}' vypise ?pocet pismen? pro obycejnou promennou 
  # a pocet prvku v poli
  
  printf -v LINEWIDTH "%*s" \
  "$((FIELDWIDTH[0]+FIELDWIDTH[1]+FIELDWIDTH[2]+FIELDWIDTH[3]+1))"

  local i=0
  printLine
  
  # Vytiskneme nadpis predchazejici vysledky testu proti referenci
  
  printf "${BOLD}${YELLOW}%s${NORMAL}\n\n" "Testy z archivu:" 
  
  for soubor in "${REFERVYS[@]}" # Iterujeme referencnimi vystupy
  do
    # Vytiskneme radky obsahujici vysledky jednotlivych testu
    
    printf "${BOLD}%*s${CYAN}%*d${NORMAL}${BOLD}%s${NORMAL}" \
    "${FIELDWIDTH[0]}" "Vystup" "${FIELDWIDTH[1]}" "$i" ":"
    
    # Budeme porovnavat, pri rozdilu inkrementujeme $chyby
    # Je pouzit diff s custom formatovanim
    # Vysledny rozdil je zapsan do pole
    
    ROZDILY[$i]="$(diff  --old-line-format \
    "${BOLD}[${CYAN}%-1dn${NORMAL}${BOLD}]${GREEN}ref${NORMAL}${BOLD}: \
${NORMAL}%l%c'\012'" --new-line-format \
    "${BOLD}[${CYAN}%-1dn${NORMAL}${BOLD}] \
${RED}ty${NORMAL}${BOLD}: ${NORMAL}%l%c'\012'" \
    --unchanged-line-format "" "$soubor" \
    "${MYVYSTUP[$i]}" 2>/dev/null)"
